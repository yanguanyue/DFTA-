...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate HAM10000 classifiers on PH2.")
    parser.add_argument(
        "--checkpoint-root",
        type=str,
    ...
    parser.add_argument(
        "--ph2-root",
        type=str,
        default="/root/autodl-tmp/data/PH2/input/img_class",
    )
    parser.add_argument(
        "--ham-root",
        type=str,
...
def write_results(results: list[EvalResult], output_dir: Path) -> None:
    ...
    csv_path = output_dir / "ph2_accuracy.csv"
    md_path = output_dir / "ph2_accuracy.md"
...
def main() -> None:
    ...
    ph2_classes = sorted([p.name for p in ph2_root.iterdir() if p.is_dir()])
    missing = [cls for cls in ph2_classes if cls not in class_to_idx]
    if missing:
        print(f"Warning: PH2 classes missing in HAM10000 mapping: {missing}")
    ...
    dataset = MappedImageDataset(ph2_root, class_to_idx, transform, limit=args.limit)
    if len(dataset) == 0:
        raise SystemExit("No images found for PH2 dataset.")
    ...
    for model_name, arch, ckpt_path in checkpoints:
        print(f"Evaluating {model_name}/{arch} on PH2...")
