...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate segmentation models on ISIC2017.")
    parser.add_argument(
        "--output-root",
        type=str,
    ...
    parser.add_argument(
        "--isic-root",
        type=str,
        default="/root/autodl-tmp/data/ISIC2017/input",
    )
    parser.add_argument(
        "--output-dir",
        type=str,
...
def build_dataset(isic_root: Path, image_size: int, limit: int | None) -> SegmentationDataset:
    image_root = isic_root / "img_class"
    mask_root = isic_root / "seg_class"
...
def write_results(results: Iterable[SegResult], output_dir: Path) -> None:
    ...
    csv_path = output_dir / "isic2017_segmentation.csv"
    md_path = output_dir / "isic2017_segmentation.md"
...
def main() -> None:
    ...
    dataset = build_dataset(isic_root, args.image_size, args.limit)
    if len(dataset) == 0:
        raise SystemExit("No ISIC2017 samples found for segmentation evaluation.")
    ...
    for dataset_name, model_type, ckpt_path in model_entries:
        print(f"Evaluating {model_type}/{dataset_name} on ISIC2017...")
        if model_type == "unet":
