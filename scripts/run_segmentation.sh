...
SYN_ROOT=/root/autodl-tmp/output/generate
MIX_ROOT=/root/autodl-tmp/output/mixed_datasets/segmentation
OUT_ROOT=/root/autodl-tmp/output/segmentation
...
root = Path("/root/autodl-tmp/output/generate")
if not root.exists():
    sys.exit(0)
...
for model_dir in sorted(root.iterdir()):
    if model_dir.is_dir() and has_masks(model_dir):
