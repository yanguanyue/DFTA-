...
import copy
from model.dual_architecture import (
    HintEncoder,
    build_control_inputs,
    build_conditioning_dicts,
    apply_dual_flow_control,
    dual_flow_loss_components,
)
...
class ControlLDM(LatentDiffusion):
    ...
    def get_input(self, batch, k, bs=None, *args, **kwargs):
        ...
        control_mask, control_image = build_control_inputs(batch, self.control_key, self.device, bs)
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        ...
        loss_bundle = dual_flow_loss_components(x_start, t, cond, self, default)
        loss_simple = loss_bundle["loss_simple"]

        self._write_loss_log(loss_bundle)
        loss_dict[f'{prefix}/loss_simple'] = loss_simple.mean()

        loss, loss_vlb = self._aggregate_loss(loss_simple, t)
    ...
    def _write_loss_log(self, loss_bundle):
        try:
            log_dir = self.trainer.default_root_dir if getattr(self, "trainer", None) else "."
            log_path = os.path.join(log_dir, "loss_log.txt")
            step = int(getattr(self, 'global_step', 0))
            mask_val = loss_bundle['loss_simple_mask_value']
            image_val = loss_bundle['loss_simple_image_value']
            m2i_val = loss_bundle['loss_simple_mask_2_image_value']
            reg_val = loss_bundle['loss_simple_mask_regularization_value']

            with open(log_path, "a", encoding="utf-8") as f:
                f.write(
                    f"step={step} "
                    f"loss_simple_mask={mask_val:.8f} "
                    f"loss_simple_image={image_val if image_val is not None else 'NA'} "
                    f"loss_simple_mask_2_image={m2i_val if m2i_val is not None else 'NA'} "
                    f"loss_simple_mask_regularization={reg_val if reg_val is not None else 'NA'}\n"
                )
        except Exception:
            pass

    def _aggregate_loss(self, loss_simple, t):
