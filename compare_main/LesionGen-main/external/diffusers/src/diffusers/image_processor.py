...
class VaeImageProcessor(ConfigMixin):
    ...
    def numpy_to_pil(images: np.ndarray) -> list[PIL.Image.Image]:
        ...
        # Avoid numpy dtype conversion issues by routing through torch
        images_t = torch.as_tensor(images)
        images_t = (images_t * 255).round().clamp(0, 255).to(torch.uint8)

        pil_images: list[PIL.Image.Image] = []
        if images_t.shape[-1] == 1:
            for image in images_t:
                image = image.squeeze(-1).contiguous()
                data = bytes(image.view(-1).tolist())
                pil_images.append(Image.frombytes("L", (image.shape[1], image.shape[0]), data))
        else:
            for image in images_t:
                image = image.contiguous()
                data = bytes(image.view(-1).tolist())
                pil_images.append(Image.frombytes("RGB", (image.shape[1], image.shape[0]), data))
