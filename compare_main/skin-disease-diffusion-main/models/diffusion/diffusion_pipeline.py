...
class DiffusionPipeline(BasicModel):
    def __init__(self, 
        ...
        self._disable_wandb_image_log = False
        ...
        if self.global_step != 0 and self.global_step % self.sample_every_n_steps == 0:
          if state == 'train' and batch_idx == 0:
            ...
            for i in range(min(4, len(images))):
                img = images[i]
                img_norm = safe_normalize(img)
                images_to_log.append(img_norm)

            if (not self._disable_wandb_image_log) and hasattr(self.logger, "log_image"):
                try:
                    ...
                    self._disable_wandb_image_log = True
                    print(f"WandB image logging disabled due to error: {exc}")

            if wandb.run is not None:
                path_out = Path(wandb.run.dir) / 'images'
                path_out.mkdir(parents=True, exist_ok=True)
            else:
                path_out = Path('images')
                path_out.mkdir(parents=True, exist_ok=True)
