#!/usr/bin/env python
# coding=utf-8
"""Train SD1.5 LoRA with T2I-Adapter conditioning (mask) on HAM10000.

- Images + masks come from CSV files (img_path, seg_path, llava_prompt).
- Uses local SD1.5 diffusers model and local T2I-Adapter weights.
"""
...
from diffusers import AutoencoderKL, DDPMScheduler, StableDiffusionPipeline, T2IAdapter, UNet2DConditionModel
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train SD1.5 LoRA with T2I-Adapter conditioning.")
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to local T2I-Adapter weights.",
    )
    parser.add_argument(
        "--csv_paths",
        type=str,
...
def main():
    ...
    adapter = T2IAdapter.from_pretrained(args.adapter_path)
    ...
    for epoch in range(num_train_epochs):
        ...
        for step, batch in enumerate(dataloader):
            with accelerator.accumulate(unet):
                ...
                adapter_state = adapter(conditioning_image)
                adapter_state = [state * args.adapter_conditioning_scale for state in adapter_state]
