...
### HAM10000 (512) + LoRA
We provide ready-to-run configs for HAM10000 at 512 resolution, including a base model and per-class LoRA fine-tuning.

1. Train the base model first:
```bash
python main.py --config_file configs/HAM10000_base_512.yaml
```

2. Copy the resulting checkpoint to a fixed path used by LoRA configs:
```bash
cp ~/.workspace/logs/ArSDM_exps/HAM10000_base_512/<timestamp>/checkpoints/last.ckpt /root/autodl-tmp/model/HAM10000_base_512.ckpt
```

3. Train LoRA per class (examples):
```bash
python main.py --config_file configs/HAM10000_akiec.yaml
python main.py --config_file configs/HAM10000_bcc.yaml
python main.py --config_file configs/HAM10000_bkl_lora_512.yaml
```

Available class configs: `akiec`, `bcc`, `bkl`, `df`, `mel`, `nv`, `vasc`.
