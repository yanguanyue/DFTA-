...
def make_transform(
    ...
    if resize_filter == 'cv2_inter_cubic':
        if cv2 is None:
            print('Warning: OpenCV not available, falling back to PIL.BICUBIC for resizing.')
        ...
        if source_mask is not None:
            ...
            for suffix in ('_mask', '_segmentation'):
                for ext in (img_ext, '.png', '.jpg', '.jpeg'):
                    candidate = os.path.join(source_mask, f'{img_idx}{suffix}{ext}')
            ...
            if mask_path is None:
                error(f'Mask not found for {image["img_name"]} under {source_mask}')
