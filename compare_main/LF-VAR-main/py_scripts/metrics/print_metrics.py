...
def main():
    ...
    for file_name in files:
        ...
        mean = metrics.get("inception_score_mean", 0.0)
        std = metrics.get("inception_score_std", 0.0)
        fid = metrics.get("FID", 0.0)
        fids.append(fid)
        means.append(mean)
        stds.append(std)
        print(file_name.replace(args.evaluate+"_", "").replace(".txt","")+":",end="")
        print(f"{mean:.2f}\\\\ ± {std:.2f}\t{fid:.2f}")
    size = len(means)
    assert size == len(fids) == len(stds)
    print("Average:")
    print(f"{sum(means)/size:.2f}\\\\ ±{sum(stds)/size:.2f}\t{sum(fids)/size:.2f}")
    print("=============")
