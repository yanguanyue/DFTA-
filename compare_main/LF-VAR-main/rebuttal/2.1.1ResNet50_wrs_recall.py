...
def create_weighted_sampler(dataframe, class_to_idx):
    ...
    for class_name in class_counts.index:
        ...
        class_weights[class_name] = total_samples / class_count
    
    sample_weights = []
    for idx in range(len(dataframe)):
        class_name = dataframe.iloc[idx]['class']
...
def create_data_loaders(csv_path, batch_size=32, val_ratio=0.2, use_weighted_sampler=True):
    ...
    if use_weighted_sampler:
        train_sampler = create_weighted_sampler(train_df, train_dataset.class_to_idx)
...
def validate(model, loader, criterion, device):
    ...
    return epoch_loss, epoch_acc, all_preds, all_labels, overall_recall, per_class_recall
...
def plot_recall_by_class(per_class_recall, class_names, output_dir):
    
    plt.figure(figsize=(10, 6))
    plt.bar(class_names, per_class_recall)
    plt.xlabel('Class')
    plt.ylabel('Recall')
    plt.title('Recall by Class')
    plt.xticks(rotation=45, ha='right')
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'recall_by_class.png'))
    plt.close()

def save_training_log(output_dir, train_losses, train_accs, val_losses, val_accs, 
                     val_recalls, val_labels, val_preds, class_names, final_acc, 
                     final_recall, per_class_recall, use_weighted_sampler):
    ...
    with open(log_path, 'w') as f:
        ...
        f.write(f"Used weighted sampling: {use_weighted_sampler}\n")
        f.write(f"Final validation accuracy: {final_acc:.2f}%\n")
        f.write(f"Final validation recall (weighted): {final_recall:.4f}\n\n")
        
        f.write("Per-class Recall:\n")
        for i, cls_name in enumerate(class_names):
            f.write(f"{cls_name}: {per_class_recall[i]:.4f}\n")
        f.write("\n")
        ...
        for epoch in range(len(train_losses)):
            ...
            f.write(f"Val Recall: {val_recalls[epoch]:.4f}\n")
...
def train_resnet50_classifier(csv_path, num_epochs=25, batch_size=32, learning_rate=0.001, 
    ...
    print(f"\nStarting training with weighted sampling: {use_weighted_sampler}")
    best_val_acc = 0.0
    best_val_recall = 0.0
    ...
    for epoch in range(num_epochs):
        ...
        val_loss, val_acc, val_preds, val_labels, val_recall, per_class_recall = validate(model, val_loader, criterion, device)
        ...
        if val_recall > best_val_recall:
            ...
            torch.save(model.state_dict(), os.path.join(output_dir, 'best_resnet50_model_recall.pth'))
    ...
    print("\nPer-class Recall:")
    for i, cls_name in enumerate(class_names):
        print(f"{cls_name}: {per_class_recall[i]:.4f}")
    ...
    plot_recall_by_class(per_class_recall, class_names, output_dir)
    
    save_training_log(output_dir, train_losses, train_accs, val_losses, val_accs,
                     val_recalls, val_labels, val_preds, class_names, val_acc, 
                     val_recall, per_class_recall, use_weighted_sampler)
    ...
    return model, val_acc, val_recall, output_dir

if __name__ == "__main__":
    csv_path = "/mnt/SkinGenerativeModel/code/data/local/Dermofit/input/metadata_rebuttal.csv"
    
    try:
        model, final_acc, final_recall, output_dir = train_resnet50_classifier(
            csv_path=csv_path,
            num_epochs=25,
            batch_size=32,
            learning_rate=0.001,
        ...
        print(f"Best validation recall: {final_recall:.4f}")
        print(f"All outputs saved to: {output_dir}")
        print("Saved outputs:")
        print("- best_resnet50_model.pth (model weights - best accuracy)")
        print("- best_resnet50_model_recall.pth (model weights - best recall)")
        print("- checkpoint_best.pth (full checkpoint)")
        print("- training_history.png (loss, accuracy and recall plots)")
        print("- confusion_matrix.png (confusion matrix)")
        print("- recall_by_class.png (per-class recall visualisation)")
