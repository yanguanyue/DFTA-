...
# Workaround for environments where torch.from_numpy fails despite valid numpy arrays.
...
def _safe_from_numpy(array):
    try:
        ...
        torch_dtype = dtype_map.get(np.dtype(array.dtype), torch.float32)
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "CSV file containing per-image prompts. When set, DreamBooth will read image paths and prompts from the"
            " CSV instead of using --instance_data_dir and --instance_prompt."
        ),
    )
    parser.add_argument(
        ...
        type=str,
        default="img_path",
        help="Column name in --metadata_csv that contains the image path.",
    )
    parser.add_argument(
        "--metadata_prompt_column",
        type=str,
        default="prompt",
        help="Column name in --metadata_csv that contains the prompt text.",
    )
    parser.add_argument(
        "--metadata_path_replace_from",
        type=str,
        default=None,
        help="Optional prefix in CSV image paths to replace.",
    )
    parser.add_argument(
        "--metadata_path_replace_to",
        type=str,
        default=None,
        help="Replacement prefix for --metadata_path_replace_from.",
    )
    parser.add_argument(
        "--class_data_dir",
        type=str,
    ...
    if args.metadata_csv is not None and args.with_prior_preservation:
        raise ValueError(
            "--metadata_csv does not support prior preservation. Please remove --with_prior_preservation."
        )

    if args.metadata_csv is None:
        if args.instance_data_dir is None:
            raise ValueError("You must specify --instance_data_dir when --metadata_csv is not set.")
        if args.instance_prompt is None:
            raise ValueError("You must specify --instance_prompt when --metadata_csv is not set.")
    ...
    if args.metadata_csv is not None and args.pre_compute_text_embeddings:
        raise ValueError(
            "--metadata_csv cannot be used with --pre_compute_text_embeddings because prompts vary per image."
        )
...
class DreamBoothMetadataDataset(Dataset):
    """
    A dataset that reads image paths and per-image prompts from a metadata CSV.
    """
    ...
    with open(csv_path, "r", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            if path_replace_from and path_replace_to:
                image_path = image_path.replace(path_replace_from, path_replace_to, 1)
...
def main(args):
    ...
    if args.metadata_csv is not None:
        metadata_records = load_metadata_csv(
            args.metadata_csv,
            args.metadata_image_column,
            args.metadata_prompt_column,
            args.metadata_path_replace_from,
            args.metadata_path_replace_to,
        )
        train_dataset = DreamBoothMetadataDataset(
